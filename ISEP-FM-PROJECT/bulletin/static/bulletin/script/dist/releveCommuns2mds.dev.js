...
window.addEventListener('load', function () {
  ...
  function Page1() {
    listStudent = mydata[1]; //console.log(listStudent);
    ...
    for (i = 0; i <= mydata[1].length - 1; i++) {
      ...
      for (j = 0; j <= tab23.childElementCount - 1; j++) {
        ...
        ; //{ {if(listStudent[i][7][5]){tab23.cells[j].innerHTML ='V'}else{tab23.cells[j].innerHTML ='NV'}};};
      } //tab3
      ...
      for (j = 0; j <= tab33.childElementCount - 1; j++) {
        ...
        if (j == 2) {
          tab33.cells[j].innerHTML = listStudent[i][9][3];
        }

        ;

        if (j == 3) {
          if (listStudent[i][9][3] >= 10) {
      ...
      for (j = 0; j <= tab43.childElementCount - 1; j++) {
        ...
        if (j == 3) {
          tab43.cells[j].innerHTML = listStudent[i][2][7] + listStudent[i][4][7] + listStudent[i][5][7] + listStudent[i][6][7] + listStudent[i][7][7] + listStudent[i][8][7] + listStudent[i][9][7] + listStudent[i][10][7];
        }
      ...
      worktab.appendChild(temp); //worktab1.appendChild(temp1)
    }
  }
